...
int main() {
	...
	*/
	...
	for (int y = 0; y < 9; y++) { //zeilen
		
		int test[9] = { 0 };//Eingabe test

		for (int x = 0; x < 9; x++) {
			test[sudoku[x][y] - 1]++; //1 eingefügt wenn zahl vorkommt (fängt bei null an daher -1)
		}
		for (int i = 0; i < 9; i++) {
	...
	for (int b = 0; b < 9; b++) { //blöcke
		
		int test[9] = { 0 };

		for (int y_b = 0; y_b < 3; y_b++) { //nur bi szur drittend danach nächste zeile
			
			for (int x_b = 0; x_b < 3; x_b++) { //nur bis zur dritten position -> block = 3x3, danach nächste zeile
				
				test[sudoku[x_b + (b % 3) * 3][y_b + (b / 3) * 3] - 1]++; //modulo bleibt bei richtigen eingaben auf nullx3 -> eintrag bleibt negativ
			}
		}
		for (int i = 0; i < 9; i++) {
			if (test[i] == 0) {
